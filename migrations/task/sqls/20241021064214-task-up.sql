...
UPDATE "USER" SET role = 'COACH', updated_at = now()  
...
-- 3-3 修改：更新教練的經驗年數，資料需求如下：
    -- 1. 教練`肌肉棒子` 的經驗年數為3年
    -- 2. 教練`Q太郎` 的經驗年數為5年
UPDATE "COACH" SET experience_years = 3, updated_at = now() 
WHERE user_id = (SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io');  

UPDATE "COACH" SET experience_years = 5, updated_at = now() 
...
SELECT user_id, sum(purchased_credits) total FROM "CREDIT_PURCHASE" 
WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
GROUP BY user_id ;
...
WHERE cp2.created_at >= '2024-12-01 00:00:00' AND cp2.created_at <= '2024-12-31 23:59:59' 
GROUP BY cp2.name;

-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
SELECT SUM(price_paid) "總營收"
FROM "CREDIT_PURCHASE" 
WHERE purchase_at >= '2024-12-01 00:00:00' AND purchase_at <= '2024-12-31 23:59:59';
-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數
SELECT COUNT(DISTINCT user_id) "預約會員人數" 
FROM "COURSE_BOOKING"
WHERE status != '課程已取消' AND created_at >= '2024-12-01 00:00:00' AND created_at <= '2024-12-31 23:59:59';
